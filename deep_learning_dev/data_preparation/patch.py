...
import data_preparation.xml_parser as xml


RESULT_DIR = "results" # "" if deactivated. Useful for Google Collab
...
class DataPatcher:
    ...
    def _copyImages(self, new_size= (None, None), anomalies= set(), extensions= ['*.jpg', '*.tif', '*.png'], verbose= True):
        """
    ...
    def patch(self, names_labels, size_img, verbose= True, debug_annotations= False):
        ...
        # Copy images
        images = self._copyImages(new_size= size_img, anomalies= anomalies, verbose= verbose)
...
class AnnotationEncoder:
    ...
    """
    def __init__(self, dir_annotations, warning= False):
        self.files = DataStructure.collectPaths(dir_annotations, '*.xml')
        self.namespaces_label = xml.collectAllTags(self.files, warning)
    ...
    def calculateStatistics(self, names_labels, debug= False):
        stats_files = dict()
        unique_labels = set()
        number_labels_file = self.countLabelsFiles(names_labels, debug)
    ...
    def countLabelsFiles(self, names_labels, debug= False):
...
class MaskBuilder:
    ...
    def buildAllMasks(self, new_size, verbose= True):
        for name, shapes_label in tqdm(self.shapes_label_file.items(), desc= _DESC_PROGRESSBAR_IMAGES, disable= not verbose):

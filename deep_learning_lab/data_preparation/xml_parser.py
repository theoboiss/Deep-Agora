...
def _parseTagsNamespace_(path: str, namespaces_tag= defaultdict(set)) -> dict:
    """
...
def _addAttributeValuesFromTag_(tag, attribute, root, namespaces, all_attributes):
...
def collectAllTags(paths_annotations: list, warning= True) -> list:
    ...
    for path in paths_annotations:
        try:
            _parseTagsNamespace_(path, namespaces_tag)
